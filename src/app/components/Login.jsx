    ...
    useEffect(() => {
        if (isOpen) {
            const getJWT = async() => {
                ...
                const response = await fetch(`https://${process.env.NEXT_PUBLIC_SERVER_IP}/api/QuickLoginJWT`);
        ...
        if (jwtChallenge) {
            const connectWebSocket = async () => {
                if (jwtChallenge) {
                    ...
                    // });
                    const response = await fetch(`https://${process.env.NEXT_PUBLIC_SERVER_IP}/api/QuickLoginSocket`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
